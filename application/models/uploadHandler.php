<?php
class UploadHandler extends CI_Model
{
	public $result;
	public $is_post;
    protected $options;
	...
	protected function get_download_url_relate($file_name, $version = null) {
        if ($this->options['download_via_php']) {
            $url = $this->options['script_url'].'?file='.rawurlencode($file_name);
            if ($version) {
                $url .= '&version='.rawurlencode($version);
        ...
        $version_path = empty($version) ? '' : rawurlencode($version).'/';
        return $this->options['upload_url_relate'].$this->get_user_path()
            .$version_path.rawurlencode($file_name);
    }
	...
	protected function process_pdf($file_path){
		$imagick = new Imagick();
		...
		foreach($imagick as $key=>$val){
			...
			$filename=$this->get_upload_path(md5($key.time()).'.png');
    ...
    protected function handle_file_upload($uploaded_file, $name, $size, $type, $error,
        ...
        $file = new stdClass();
        $file->origin_name= $name;// iconv("utf-8", "gb2312", $name);;
//        $file->name = $this->trim_file_name($name, $type, $index, $content_range);
        ...
        if ($this->validate($uploaded_file, $file, $error, $index)) {
            ...
            $file_new_name = md5($file->name.time()).'.'.strtolower(substr(strrchr($name, '.'), 1));
            $file_path = $this->get_upload_path($file_new_name);
...
//                $file->url = $this->get_download_url($file->name);
//                $file->url_relate = $this->get_download_url_relate($file->name);
   				$file->url = $this->get_download_url($file_new_name);
				$file->url_relate = $this->get_download_url_relate($file_new_name);
                /*
                if ($type=="application/pdf"){
                	$result = $this->process_pdf($file_path);
                	$file->media_url_relate = $result;	
                }*/
                foreach($this->options['image_versions'] as $version => $options) {
                	$result =$this->create_scaled_image($file_new_name, $version, $options);
                    ...
                    if ($result['status']) {
                        if (!empty($version)) {
                            ...
                            $file->{$version.'_url_relate'} = $this->get_download_url_relate(
                                $file_new_name,
                                $version
                            );
